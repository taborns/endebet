            ...
            <div class="block-body">
                ...
                <div class="block-right">
                    ...
                    <div class="block-col block-col-33 block-accomodation">
                        ...
                        <dl>
                            <?php 
                            if($acc['acc_guests'] > 1) { $guests = esc_attr(homey_option('sn_acc_guests_label')); } else { $guests = esc_attr(homey_option('sn_acc_guest_label')); }

                            if(!empty($acc['acc_bedroom_name'])) {
                                echo '<dt>'.esc_attr($acc['acc_bedroom_name']).'</dt>';
                            }
                            if(!empty($acc['acc_no_of_beds']) || !empty($acc['acc_bedroom_type'])) {
                                echo '<dd>'.esc_attr($acc['acc_no_of_beds']).' '.esc_attr($acc['acc_bedroom_type']).'</dd>';
                            }
                            if(!empty($acc['acc_guests'])) {
                                //echo '<dt>'.$acc['acc_guests'].' '.$guests.'/dt>';
                                echo '<dd>'.esc_attr($acc['acc_guests']).' '.esc_attr($guests).'</dd>';
                            }
                            ?>
                        </dl>                    
                    </div>
